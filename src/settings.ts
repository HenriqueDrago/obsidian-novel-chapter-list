...
export const DEFAULT_SETTINGS: NovelChapterPluginSettings = {
  novelProjects: [{ id: `default-${Date.now()}`, name: "My First Novel", path: "Novel/Chapters" }],
...
export class NovelChapterPluginSettingsTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    new Setting(containerEl)
      ...
      .setDesc('The actual frontmatter key to update (e.g., "status", "outlineLevel"). This is case-sensitive.')
      .addText(text => text
        .setPlaceholder(DEFAULT_SETTINGS.propertyNameToChange)
        .setValue(this.plugin.settings.propertyNameToChange)
        .onChange(async (value) => {
          this.plugin.settings.propertyNameToChange = value.trim();
    ...
    new Setting(containerEl)
      ...
      .setDesc('What title to display for the property column in the view (e.g., "Current Status"). If empty, it will use the "Target Property Name".')
      .addText(text => text
        .setPlaceholder("e.g., Status (optional)")
        .setValue(this.plugin.settings.propertyColumnHeader)
        .onChange(async (value) => {
          this.plugin.settings.propertyColumnHeader = value.trim();
          await this.plugin.saveSettings();
          this.plugin.refreshNovelChapterView();
        }));
    ...
    new Setting(containerEl)
      ...
      .setDesc('Enter a comma-separated list of options for the dropdown (e.g., "To Do,In Progress,Complete").')
      .addText(text => text
    ...
    new Setting(containerEl)
        ...
        .setDesc('Path to a markdown file to use as a template for new chapters (e.g., "Templates/ChapterTemplate.md"). Leave empty to not use a template. Placeholders like {{title}} and {{date}} can be used in the template.')
        .addText(text => text
            .setPlaceholder(DEFAULT_SETTINGS.chapterTemplatePath || "e.g., Templates/ChapterTemplate.md")
    ...
    const novelProjectsContainer = containerEl.createDiv({ cls: 'novel-projects-container' });
      ...
      .addButton(button => button
        ...
        .onClick(async () => {
          this.plugin.settings.novelProjects.push({
            id: generateId(),
            name: `New Novel ${this.plugin.settings.novelProjects.length + 1}`,
            path: ''
          });
          await this.plugin.saveSettings();
          this.renderNovelProjects(novelProjectsContainer); 
  ...
  private renderNovelProjects(containerEl: HTMLElement): void {
    ...
    if (this.plugin.settings.novelProjects.length === 0) {
      containerEl.createEl('p', { text: 'No novel projects configured. Add one below.' });
    }
    ...
    this.plugin.settings.novelProjects.forEach((project, index) => {
      ...
      projectSetting.addText(text => text
        .setPlaceholder('Folder Path (e.g., Novels/MyEpicSaga/Chapters)')
