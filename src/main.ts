import { Plugin, WorkspaceLeaf, Notice, ItemView, TFile, normalizePath, Modal, TextComponent, ButtonComponent, App, Setting } from "obsidian";
...
class ChapterNameModal extends Modal {
    ...
    constructor(app: App, title: string, ctaText: string, initialValue: string, onSubmit: (result: string) => void) {
        super(app);
...
class NovelChapterView extends ItemView {
  ...
  async renderControls(): Promise<void> {
    ...
    const controlsLine1 = this.controlsContainer.createDiv({ cls: 'novel-chapter-controls-line' });
    const controlsLine2 = this.controlsContainer.createDiv({ cls: 'novel-chapter-controls-line' });
    ...
    if(optionsArray.length > 0) {
        ...
        filterSelect.createEl('option', { text: 'Filter by property...', value: 'all' });
        optionsArray.forEach(option => {
            filterSelect.createEl('option', { text: option, value: option });
        });
  ...
  async renderChapterTable(): Promise<void> {
    ...
    if (!chaptersFolderPath || chaptersFolderPath.trim() === "") {
      this.tableContainer.setText(`Project "${currentNovelProject.name}" has no folder path configured.`);
      return;
    }

    const files = this.app.vault.getMarkdownFiles();
    let chapterFiles = files.filter(file => isFileInFolder(file, chaptersFolderPath));

    // Apply Filters
    if (this.filterValue !== 'all') {
        chapterFiles = chapterFiles.filter(file => {
            const fileCache = this.app.metadataCache.getFileCache(file);
            const propValue = fileCache?.frontmatter?.[propertyNameToChange];
            return propValue === this.filterValue;
        });
    }
    if (this.searchTerm) {
        chapterFiles = chapterFiles.filter(file => file.basename.toLowerCase().includes(this.searchTerm.toLowerCase()));
    }
    
    chapterFiles.sort((a, b) => a.path.localeCompare(b.path));

    if (chapterFiles.length === 0) {
      this.tableContainer.setText(`No chapters found in "${chaptersFolderPath}" matching your criteria.`);
      return;
    }
    ...
    if (propertyNameToChange && optionsArray.length > 0) {
        const propertyHeader = propertyColumnHeader.trim() || propertyNameToChange;
        headerRow.createEl('th', { text: propertyHeader });
    }
    headerRow.createEl('th', { text: 'Actions' }).addClass('actions-header');

    const tbody = table.createEl('tbody');
    for (const file of chapterFiles) {
      const row = tbody.createEl('tr');
      const chapterCell = row.createEl('td');
      const chapterLink = chapterCell.createEl('a', { text: file.basename, href: '#', cls: 'internal-link' });
      ...
      if (propertyNameToChange && optionsArray.length > 0) {
          ...
          selectEl.addEventListener('change', async (event) => {
            ...
            await this.plugin.updateChapterProperty(file, propertyNameToChange, newValue);
          });
      }
...
export default class NovelChapterPlugin extends Plugin {
  ...
  private findProjectForFile(file: TFile): NovelProject | null {
    if (!file) return null;
    return this.settings.novelProjects.find(p => p.path && isFileInFolder(file, p.path)) || null;
  }

  // Method to update a chapter's frontmatter property
  public async updateChapterProperty(file: TFile, propertyName: string, newValue: string): Promise<void> {
    try {
        ...
        new Notice(`Failed to update ${propertyName} for ${file.basename}.`);
    }
  }
  ...
  public async promptAndRenameChapter(file: TFile): Promise<void> {
    ...
    new ChapterNameModal(this.app, "Rename Chapter", "Rename", currentName, async (newName) => {
...
private async renameChapter(file: TFile, newName: string): Promise<void> {
    ...
    if (await this.app.vault.adapter.exists(normalizePath(newPath))) {
        new Notice(`A chapter named "${sanitizedFileName}.md" already exists.`);
        return;
    }
    try {
  ...
  public async promptAndCreateNewChapter(projectId: string | null): Promise<void> {
    ...
    if (!currentNovelProject || !currentNovelProject.path?.trim()) {
        new Notice('The selected project does not have a valid folder path configured.');
        return;
    }

    new ChapterNameModal(this.app, "Enter New Chapter Name", "Create Chapter", "", async (chapterName) => {
        ...
        if (!await this.app.vault.adapter.exists(folderPath)) {
             new Notice(`Project folder "${folderPath}" does not exist.`);
             return;
        }
        ...
        if (templatePath) {
            ...
            if (templateFile instanceof TFile) {
                ...
                new Notice(`Template file not found at "${templatePath}". Creating a default chapter.`);
            }
        }
        ...
        if (fileContent === "") {
            ...
            fileContent = `---\ntitle: ${chapterName}\n`;
            if (propertyNameToChange) fileContent += `${propertyNameToChange}: ""\n`;
            fileContent += `---\n\n# ${chapterName}\n\n`;
        }
  ...
  async onload() {
    ...
    this.addCommand({
        ...
        callback: () => {
            ...
            if (!targetProjectId) {
                new Notice("Cannot determine project. Please select a project in the Novel Chapters view.");
                return;
            }
    ...
    this.addRibbonIcon('list-ordered', 'Open Novel Chapter View', () => this.activateView());
  ...
  async loadSettings() {
    ...
    if (!Array.isArray(this.settings.novelProjects) || this.settings.novelProjects.length === 0) {
        this.settings.novelProjects = [{ id: `default-load-${Date.now()}`, name: "Default Novel", path: "" }];
    }
    ...
    if (this.settings.lastSelectedNovelId && !this.settings.novelProjects.find(p => p.id === this.settings.lastSelectedNovelId)) {
        this.settings.lastSelectedNovelId = this.settings.novelProjects.length > 0 ? this.settings.novelProjects[0].id : null;
    }
  }
  ...
  private async handleFileChange(file: TFile, oldPath?: string) {
    ...
    const isRelevantChange = this.settings.novelProjects.some(project => {
        ...
        const inCurrentPath = normalizePath(file.path).startsWith(projectPath + '/');
        const inOldPath = oldPath ? normalizePath(oldPath).startsWith(projectPath + '/') : false;
